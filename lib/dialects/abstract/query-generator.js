...
const QueryGenerator = {
  ...
  */
  quote(collection, parent, connector, queryOptions) {
    ...
    if (typeof collection === 'string') {
      return this.quoteIdentifiers(collection, queryOptions);
  ...
  /*
   ...
   @private
  */
  quoteIdentifiers(identifiers, options) {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.group) {
      options.group = Array.isArray(options.group) ? options.group.map(t => this.quote(t, model, undefined, options)).join(', ') : this.quote(options.group, model, undefined, options);
      if (subQuery) {
  ...
  whereItemQuery(key, value, options) {
    ...
    if (!value) {
      return this._joinKeyValue(key, this.escape(value, field), value === null ? this.OperatorMap[Op.is] : this.OperatorMap[Op.eq], options.prefix, options);
    }
    ...
    if (isArray && fieldType instanceof DataTypes.ARRAY) {
      return this._joinKeyValue(key, this.escape(value, field), this.OperatorMap[Op.eq], options.prefix, options);
    }
    ...
    if (key === Op.placeholder) {
      return this._joinKeyValue(this.OperatorMap[key], this.escape(value, field), this.OperatorMap[Op.eq], options.prefix, options);
    }
    return this._joinKeyValue(key, this.escape(value, field), this.OperatorMap[Op.eq], options.prefix, options);
  },
  ...
  _joinKeyValue(key, value, comparator, prefix, options) {
    if (!key) {
      return value;
    }
    ...
    key = this._getSafeKey(key, prefix, options);
    return [key, value].join(' '+comparator+' ');
  },

  _getSafeKey(key, prefix, options) {
    ...
    if (Utils.isColString(key)) {
      ...
      var _findSub = function(key){
        var str = key;
        var splitKey = key.split('->');
        if(splitKey.length == 1){
          const includeIndex = options.include.findIndex(include => key === include.as);
          if(includeIndex !== -1){
              const includeAs = options.include[includeIndex];
              if(includeAs.substituteAs){
                str = includeAs.substituteAs;
              }
          }
        } else {
          const includeIndex = options.include.findIndex(include => splitKey[0] === include.as);
          if(includeIndex !== -1){
            const includeMap = options.include[includeIndex].includeMap;
            if(includeMap && includeMap[splitKey[splitKey.length - 1]]){
              const includeAs = includeMap[splitKey[splitKey.length - 1]];
              if(includeAs.substituteAs){
                str = includeAs.substituteAs;
              }
            }
      ...
      if (key.length > 2) {
        key = [
          ...
          _findSub(key.slice(0, -1).join('->')),
          key[key.length - 1]
        ];
      } else {
        key[0] = _findSub(key[0]);
      }
  ...
  _whereParseSingleValueObject (key, field, prop, value, options) {
    ...
    switch (prop) {
      ...
      case Op.notIn:
        if (value instanceof Utils.Literal) {
          return this._joinKeyValue(key, value.val, comparator, options.prefix, options);
        }

        if (value.length) {
          return this._joinKeyValue(key, `(${value.map(item => this.escape(item, field)).join(', ')})`, comparator, options.prefix, options);
        }

        if (comparator === this.OperatorMap[Op.in]) {
          return this._joinKeyValue(key, '(NULL)', comparator, options.prefix, options);
        }
      ...
      case Op.all:
        ...
        if (value[Op.values]) {
          return this._joinKeyValue(key, `(VALUES ${value[Op.values].map(item => `(${this.escape(item)})`).join(', ')})`, comparator, options.prefix, options);
        }

        return this._joinKeyValue(key, `(${this.escape(value, field)})`, comparator, options.prefix, options);
      case Op.between:
      case Op.notBetween:
        return this._joinKeyValue(key, `${this.escape(value[0])} AND ${this.escape(value[1])}`, comparator, options.prefix, options);
      case Op.raw:
      ...
      case Op.col:
        ...
        return this._joinKeyValue(key, value.map(identifier => this.quoteIdentifier(identifier)).join('.'), comparator, options.prefix, options);
    }
    ...
    if (_.isPlainObject(value)) {
      if (value[Op.col]) {
        return this._joinKeyValue(key, this.whereItemQuery(null, value), comparator, options.prefix, options);
      }
      if (value[Op.any]) {
        escapeOptions.isList = true;
        return this._joinKeyValue(key, `(${this.escape(value[Op.any], field, escapeOptions)})`, `${comparator} ${this.OperatorMap[Op.any]}`, options.prefix, options);
      }
      if (value[Op.all]) {
        escapeOptions.isList = true;
        return this._joinKeyValue(key, `(${this.escape(value[Op.all], field, escapeOptions)})`, `${comparator} ${this.OperatorMap[Op.all]}`, options.prefix, options);
      }
    }

    if (comparator.indexOf(this.OperatorMap[Op.regexp]) !== -1) {
      return this._joinKeyValue(key, `'${value}'`, comparator, options.prefix, options);
    }

    if (value === null && comparator === this.OperatorMap[Op.eq]) {
      return this._joinKeyValue(key, this.escape(value, field, escapeOptions), this.OperatorMap[Op.is], options.prefix, options);
    } else if (value === null && this.OperatorMap[Op.ne]) {
      return this._joinKeyValue(key, this.escape(value, field, escapeOptions), this.OperatorMap[Op.not], options.prefix, options);
    }

    return this._joinKeyValue(key, this.escape(value, field, escapeOptions), comparator, options.prefix, options);
  },
